...
### Added
- Added optional `dg_prefix` setting in Function 0 to prepend up to 4 letters/numbers to newly generated IDs, producing `<prefix>_dg_<epoch>` while keeping blank-prefix behavior compatible with legacy `dg_<epoch>` IDs.

### Changed
- New IDs generated by Functions 1 and 2, including compound parent IDs, now use the configured `dg_prefix` when present while preserving all existing mapped IDs.
- Function 0 settings save now preserves internal fields such as `file_to_id_map`, preventing accidental loss of previously assigned identifiers.

### Documentation
- Documented epoch cutoff `1782237851` as the introduction point for optional prefixed IDs so legacy `dg_<epoch>` values can be distinguished from post-change IDs.
